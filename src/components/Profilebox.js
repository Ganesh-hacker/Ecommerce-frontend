...
export default function Profilebox(){
  ...
  useEffect(() => {
    ...
    fetch(`${backendurl}/login/${username}`)
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            if (data.length > 0) {
                setBackendData(data[0]); // Assuming only one user is returned
            } else {
